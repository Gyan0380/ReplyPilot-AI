...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        // 1. Database se check karo ki yeh token kis user ka hai aur uski settings kya hain[cite: 1]
        ...
        // 2. Check karo ki is specific user ka bot ON hai ya OFF[cite: 1]
        if (!settings.is_active) {
            return res.status(200).json({ reply: null }); 
        }

        // 3. Mode aur Auto-Location Inference ke sath Smart System Prompt[cite: 1]
        ...
        // 4. Gemini AI ko call karo[cite: 1]
...
// Vite/Railway Compatibility[cite: 1]
